...
{	
	...
	public class HealthBar : MonoBehaviour 
	{
		...
		[Information("Add this component to an object and it'll add a healthbar next to it to reflect its health level in real time. You can decide here whether the health bar should be drawn automatically or use a prefab.",BrendonBanville.Tools.InformationAttribute.InformationType.Info,false)]
		...
		[Information("Select a prefab with a progress bar script on it. There is one example of such a prefab in Common/Resources/GUI.",BrendonBanville.Tools.InformationAttribute.InformationType.Info,false)]
		/// the prefab to use as the health bar
		public ProgressBar HealthBarPrefab;

		[Header("Drawn Healthbar Settings ")]
		[Information("Set the size (in world units), padding, back and front colors of the healthbar.",BrendonBanville.Tools.InformationAttribute.InformationType.Info,false)]
        ...
        // Need to implement a 3D version of the follow target class
		...
		[Header("Offset")]
		[Information("Set the offset (in world units), relative to the object's center, to which the health bar will be displayed.",BrendonBanville.Tools.InformationAttribute.InformationType.Info,false)]
		/// the offset to apply to the healthbar compared to the object's center
		public Vector3 HealthBarOffset = new Vector3(0f,1f,0f);

		[Header("Display")]
		[Information("Here you can define whether or not the healthbar should always be visible. If not, you can set here how long after a hit it'll remain visible.",BrendonBanville.Tools.InformationAttribute.InformationType.Info,false)]
