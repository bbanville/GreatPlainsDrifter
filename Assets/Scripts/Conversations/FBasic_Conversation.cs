...
namespace FIMSpace.FBasics
{
    ...
    public class FBasic_Conversation : MonoBehaviour
    {
        ...
        [Tooltip("Reference to switcher for conversation system")]
        public GameObject ConversationSwitcher;
            ...
            if (IsWorking)
            {
                ...
                else
                {
                    for (int i = 0; i < generatedDialogueOptions.Count; i++)
                    {
                        ...
                        {
                            ClickedOption(generatedDialogueOptions[0]);
                        }

                        if (Input.GetKeyDown(KeyCode.Alpha2))
                        {
                            ClickedOption(generatedDialogueOptions[1]);
                        }

                        if (Input.GetKeyDown(KeyCode.Alpha3))
                        {
                            ClickedOption(generatedDialogueOptions[2]);
                        }

                        if (Input.GetKeyDown(KeyCode.Alpha4))
                        {
                            ClickedOption(generatedDialogueOptions[3]);
                        }
                    }
                }
        ...
        public virtual void EndConversation()
        {
            if (IsWorking)
            {
                ...
                if (FadeToLevel)
                {
                    LevelFadeController.Instance.SceneToLoad = LevelToFadeTo;
                    LevelFadeController.Instance.FadeOutOfLevel();
                }

                var startConvo = ConversationSwitcher.GetComponent<StartConversation>();

                if (startConvo && !startConvo.StartOnSceneLoad)
                {
                    startConvo.Player.GetComponent<ThirdPersonUserControl>().readPlayerInput = true;
                    startConvo.Player.GetComponentInChildren<SkinnedMeshRenderer>().gameObject.SetActive(true);
                    startConvo.TPCamera.SetActive(true);
                    startConvo.DialogueCamera.SetActive(false);
                }
            }
        }
        ...
        public virtual void GenerateDialogueOptions(List<FBasic_DialogueBase> dialogues)
        {
            ...
            for (int i = 0; i < dialogues.Count; i++)
            {
                ...
                TextMeshProUGUI optionText = newOption.GetComponent<TextMeshProUGUI>();
